class RecordController < ApplicationController
    ...
    def itempost
    	...
    	params[:item].each do |item|
	   		@user = User.find(:all,:conditions => { :username => [item["Username"]]})
	   		if @user.size != 0
	   		 @user.each do |user|
	   		   items = Item.destroy_all(['user_id == ?',user.id])
	   		   if user.encrypted_password == 'jet31415'
	   		    p 'success'
	   		   else
	   		    p 'fales'
	   		   end 
    	...
    	@item = Item.new
  		...
  		#@user = User.find(:all,:conditions => { :username => [item["Username"]]})
  		#$userid = User.where( :username => [item["Username"]]).select('id')
  		@user.each do |user|
