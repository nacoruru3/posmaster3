...
#    @Salesmei = current_user.salesmeis.find(:all,:conditions => ["billno = ? and outlet = ?", params[:id] ,params[:id2]])
	...
	sql = ["SELECT salesmeis.billno,salesmeis.itemcode,salesmeis.itemname,salesmeis.itemprice,salesmeis.flg,salesmeis.value as salesmeisvalue,zaikos.value as zaikosvalue FROM salesmeis LEFT JOIN zaikos ON salesmeis.itemcode = zaikos.code WHERE billno = ? and outlet = ? and salesmeis.user_id = ? " ,params[:id],params[:id2],$userid]
